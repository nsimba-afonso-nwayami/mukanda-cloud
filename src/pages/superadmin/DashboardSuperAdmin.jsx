      ...
      <SuperAdminLayout title="Dashboard SuperAdmin">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          ...
          <div className="bg-slate-900 border border-blue-900 rounded-xl p-6">
            <h2 className="text-sm font-bold text-cyan-500 mb-4 uppercase tracking-wider">Atividades Recentes</h2>
            <ul className="divide-y divide-blue-900">
              {atividades.map((a, index) => (
                <li key={index} className="py-3">
                  <p className="text-sm text-slate-300">
                    <span className="text-white font-semibold">{a.user}</span> {a.action} <span className="text-cyan-300">{a.file}</span>
                  </p>
          ...
          {/* ARQUIVOS */}
          <div className="bg-slate-900 border border-blue-900 rounded-xl p-6">
            <h2 className="text-sm font-bold text-cyan-500 mb-4 uppercase tracking-wider">Arquivos Recentes</h2>
            <ul className="divide-y divide-blue-900">
            ...
            <Link to="/dashboard/superadmin/arquivos" className="mt-4 inline-block text-sm font-semibold text-cyan-500 hover:text-cyan-300 transition">Ver todos →</Link>
          </div>
        </div>

        {/* MODAL NOVA PASTA */}
        <ModalSmall isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Criar Nova Pasta" icon="fas fa-folder-plus">
